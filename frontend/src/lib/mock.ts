// Mock data layer — no backend. Seeded sessions, drifting score, canned replies.
...
const TOPIC_POOL = [
  "work stress", "family", "sleep", "self-talk", "boundaries",
  "grief", "uncertainty", "burnout", "friendship", "creative block",
  "body image", "money", "loneliness", "transitions",
];
...
export function generateSessions(count = 28): Session[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    out.push({
      id: `s-${i}`,
      date: new Date(now - i * 24 * 60 * 60 * 1000 - Math.floor(rand() * 8) * 3600_000).toISOString(),
